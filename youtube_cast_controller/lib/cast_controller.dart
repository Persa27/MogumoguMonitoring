...
class YouTubeCastBridge {
  ...
  static Future<void> sendPlayPauseCommand() async {
    try {
      ...
      throw Exception('YouTube再生制御に失敗しました: $e');
    }
  }
  
  /// YouTubeアプリにシークコマンドを送信
  static Future<void> sendSeekCommand(int positionSeconds) async {
    try {
      await _channel.invokeMethod('sendSeekCommand', {'position': positionSeconds});
    } catch (e) {
      throw Exception('YouTubeシークに失敗しました: $e');
    }
  }
  
  /// Cast中のYouTube動画情報を取得
  static Future<Map<String, dynamic>?> getCastVideoInfo() async {
    try {
      final result = await _channel.invokeMethod('getCastVideoInfo');
      if (result != null) {
        return Map<String, dynamic>.from(result);
      }
      return null;
    } catch (e) {
      developer.log('Cast動画情報の取得に失敗: $e');
      return null;
    }
  }
}
...
class MediaInfo {
  ...
  factory MediaInfo.fromMap(Map<String, dynamic> map) {
    return MediaInfo(
      ...
      state: _parsePlaybackState(map['state']),
    );
  }
  
  static PlaybackState _parsePlaybackState(dynamic state) {
